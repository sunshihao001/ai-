...
description: Method Wheel wrapper draft for GitHub Spec Kit `clarify` command. Status: enabled-draft. Port: A.
source: github/spec-kit/templates/commands/clarify.md
status: enabled-draft
port: A
---

# speckit-clarify — Method Wheel Wrapper Draft
...
- Method Wheel port: `A`
- Command guardrail: Ask only high-impact questions and write accepted answers back to durable spec.

## Upstream Command Metadata

- Upstream description: Identify underspecified areas in the current feature spec by asking up to 5 highly targeted clarification questions and encoding answers back into the spec.
- Upstream template: `templates/commands/clarify.md`
...
Note: This clarification workflow is expected to run (and be completed) BEFORE invoking `__UPSTREAM_SPECKIT_COMMAND_PLAN__`. If the user explicitly states they are skipping clarification (e.g., exploratory spike), you may proceed, but must warn that downstream rework risk increases.
...
1. Run `{SCRIPT}` from repo root **once** (combined `--json --paths-only` mode / `-Json -PathsOnly`). Parse minimal JSON payload fields:
   ...
   - If JSON parsing fails, abort and instruct user to re-run `__UPSTREAM_SPECKIT_COMMAND_SPECIFY__` or verify feature branch environment.
...
- If spec file missing, instruct user to run `__UPSTREAM_SPECKIT_COMMAND_SPECIFY__` first (do not create a new spec here).
...
- If any Outstanding or Deferred remain, recommend whether to proceed to `__UPSTREAM_SPECKIT_COMMAND_PLAN__` or run `__UPSTREAM_SPECKIT_COMMAND_CLARIFY__` again later post-plan.
