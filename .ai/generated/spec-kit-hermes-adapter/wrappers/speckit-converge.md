...
description: Method Wheel wrapper draft for GitHub Spec Kit `converge` command. Status: enabled-draft. Port: E→D.
source: github/spec-kit/templates/commands/converge.md
status: enabled-draft
port: E→D
---

# speckit-converge — Method Wheel Wrapper Draft
...
- Method Wheel port: `E→D`
- Command guardrail: Append-only repair task generation after checker finding.

## Upstream Command Metadata

- Upstream description: Assess the current codebase against the feature's spec, plan, and tasks, then append any remaining unbuilt work as new tasks to tasks.md so implement can complete it.
- Upstream template: `templates/commands/converge.md`
...
`__UPSTREAM_SPECKIT_COMMAND_IMPLEMENT__` can complete it. This command MUST run only after
`__UPSTREAM_SPECKIT_COMMAND_IMPLEMENT__` has run on the current `tasks.md`, and after `__UPSTREAM_SPECKIT_COMMAND_TASKS__` has produced a complete `tasks.md`.
...
- modify, create, or delete any application code — completing the appended tasks is the
  job of `__UPSTREAM_SPECKIT_COMMAND_IMPLEMENT__`.
...
prerequisite command to run (`__UPSTREAM_SPECKIT_COMMAND_SPECIFY__` for a missing spec, `__UPSTREAM_SPECKIT_COMMAND_PLAN__` for a missing plan,
`__UPSTREAM_SPECKIT_COMMAND_TASKS__` for missing tasks). Do not produce partial output.
...
- On `tasks_appended`: state how many tasks were appended under which phase, and recommend
  running `__UPSTREAM_SPECKIT_COMMAND_IMPLEMENT__` to complete them; note that a follow-up converge
